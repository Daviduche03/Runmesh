	...
	return (
		<section id="lifecycle" className={`scroll-mt-14 border-b border-[#15181d] ${sectionPadding}`}>
			<div className="mx-auto w-[min(1256px,calc(100%_-_48px))]">
				...
				>
					Orchestrate agent runs end to end.{" "}
					<span className="text-[#8f949e]">
						Trigger workflows, chain tool calls, and follow every action, retry, approval, and result from one place.
					</span>
				</motion.h2>
				...
				>
					<LinearBentoCard fig="FIG 1.1" title="Trigger agent runs" copy="Start a run from the dashboard, a webhook, or a schedule. Each step becomes durable queued work.">
						<MiniDashboardVisual />
					</LinearBentoCard>
					<LinearBentoCard fig="FIG 1.2" title="Execute tool calls" copy="Send JSON to your app routes, internal APIs, and webhooks while Runmesh records the response and status.">
						<TriageVisual />
					</LinearBentoCard>
					<LinearBentoCard fig="FIG 1.3" title="Track every state" copy="Follow tasks and workflow runs as they move through queued, running, completed, failed, or cancelled.">
						<AgentsVisual />
					</LinearBentoCard>
				</motion.div>
			</div>
