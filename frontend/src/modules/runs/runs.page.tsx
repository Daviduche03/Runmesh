...
import { useRunsStore, type Task } from "@/lib/stores/runs-store";
...
export function RunsPage() {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			await createTask({ url: url.trim(), payload: parsedPayload, execution_type: trigger });
			setModalOpen(false);
			setUrl("");
			setPayload("{}");
		...
		<div className="flex flex-col gap-4">
			...
			<Card>
				<CardHeader>
					<div className="flex flex-wrap items-start justify-between gap-3">
						<div className="space-y-1.5">
							...
							<CardDescription>Every durable execution and its outcome.</CardDescription>
						</div>
						<SegmentedControl
							options={statusOptions}
							value={(statusFilter ?? "all") as (typeof statusOptions)[number]["value"]}
				...
				<CardContent className="px-0">
					<Table>
						<TableHeader>
							<TableRow>
								<TableHead className="ps-4">Run</TableHead>
						...
						<TableBody>
							{loading && tasks.length === 0 ? (
								Array.from({ length: 5 }).map((_, index) => <RunSkeletonRow key={index} />)
							) : tasks.length === 0 ? (
								<TableRow>
								...
								<>
									{tasks.map((run: Task) => (
										<TableRow className="h-12" key={run.id}>
											<TableCell className="ps-4 font-mono text-[12.5px] font-medium tabular-nums">
												{run.id.slice(0, 8)}
											</TableCell>
											<TableCell className="max-w-48 truncate font-mono text-[12.5px] text-muted-foreground">
												{run.endpoint}
											</TableCell>
											<TableCell className={`text-[12.5px] font-medium ${statusColor(run.status)}`}>
												{run.status}
											</TableCell>
											<TableCell className="text-[13px] text-muted-foreground">{run.scheduled}</TableCell>
			...
			{total > 0 ? (
				<div className="flex items-center justify-between text-[13px] text-muted-foreground">
					<span>
						Showing {tasks.length} of {total} runs
					</span>
